...
def add_task(req,task, db):
    ...
    print("title, description, task_name, etc, due_date, user_id",title, description, task_name, etc, due_date, user_id )
    ...
    db_task = Tasks(user_id=user_id, title=title, task=task_name, description=description, 
                   effort_to_complete=etc, due_date=due_date)
    db.add(db_task)
    db.commit()
    db.refresh(db_task)
...
def update_task(req, update_body ,db):
    ...
    db_task.task = new_status
    db.commit()
    db.refresh(db_task)
    
    return {"message": f"Task status updated to {new_status}", "updated_status": new_status}
...
def delete_task(req, db, task):
    title = task.title
    user_id = req.state.user.id
    
    if not title:
        raise HTTPException(status_code=400, detail="Title is required in body")
